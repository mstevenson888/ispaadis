isa.epicurve <- function (dat, model, stats = FALSE, type = "cumulative", quant = c(0.25, 0.75), format = "long"){
  ...
  if (stats == TRUE) {
    ...
    rval.df <- data.frame(day.seq, rmean, rsd, rmed, rq01, rq02, rmin, rmax)
    names(rval.df) <- c("day", "pr.mean", "pr.sd", "pr.median", q01.n, q02.n, "pr.min", "pr.max")
  }
  ...
  if(format == "long"){
    rval.df <- gather(data = rval.df, key = stat, value = n, -c(day))
  }
